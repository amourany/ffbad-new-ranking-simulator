...
const fetchPlayerRankings = async (licence: number, topPlayerFetcher: (discipline: number) => Promise<number>): Promise<PlayerInfo> => {
	...
	const rankings = await convertToRankings(isCompetitivePlayer ? response.data : nonCompetitivePlayer, gender, topPlayerFetcher);
